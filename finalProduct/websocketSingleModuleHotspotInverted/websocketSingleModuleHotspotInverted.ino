...
void loop() {
  ...
  while(bothReady == 0)
  {
    ...
    if(physicalReady == '1' && remoteReady == '1' || restart = '1'){
  ...
  if(currPlayer == '0'){
    ...
    //Send data to mega
    //Invert linear actuator position
    dataServer2 = dataServer;
    LinActPos = dataServer2[3];
    LinActPosInv = (LinActPos - 51)*(-1) + 51;
    dataServer2[3] = LinActPosInv;
    Serial.println(dataServer2);
    Serial2.println(dataServer2);
...
void parseData(String dataServer){
  ...
  winner = dataServer[2];
  LinActPosInv = int(dataServer[3]);
  LinActPos = (LinActPosInv - 51)*(-1) + 51;
  dataServer[3] = LinActPos;
...
void createDataPacket(){
  dataMega = dataServer;
  sensorValueInv = int(sensorValue[0]);
  sensorValue[0] = (sensorValueInv - 51)*(-1) + 51;
